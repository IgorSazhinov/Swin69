...
export const GameActions = ({ status, isChoosingColor, previewCard, winnerName, onAction }: GameActionsProps) => {
  // Генерируем уникальный ключ для превью карты
  const previewKey = previewCard ? (previewCard.instanceId || previewCard.id || `preview-${Date.now()}`) : "no-preview";
      ...
      <AnimatePresence mode="wait">
        ...
        {status === 'FINISHED' && (
          <motion.div 
            ...
            exit={{ opacity: 0 }}
            className="w-full bg-white p-10 rounded-[50px] text-center text-black shadow-2xl border-[6px] border-orange-500"
          >
